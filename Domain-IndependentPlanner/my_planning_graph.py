...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            is_possible = False
            parent_nodes = set()
#            print('----------------------Inside add_action_level----------------------------')
#            print(type(action))
            action_node = PgNode_a(action)
            for al in action_node.prenodes:
                for l in self.s_levels[level]:
#                    print('Literal show:')
#                    l.show()
#                    print('Action show:')
#                    al.show()
                    if al.__eq__(l):
                        ...
                        parent_nodes.add(l)
            if is_possible:
                poss_actions.add(action_node)
                action_node.parents = parent_nodes
                for parent in parent_nodes:
                    parent.children.add(action_node)
#                if self.serial:
#                    break
        self.a_levels.append(poss_actions)
#        print('------------------------ Action:',level,'--------------------------')
#        tmp = []
#        for a in self.a_levels[level]:
#            tmp.append(str(a.action))
#        print(tmp)
#        for lL in self.s_levels:
#            for l in lL:
#                l.show()
...
#            print('-------Action Nodes----------',)
 #           action_node.show()
            ...
            child_nodes = set()
#            if level > 2:
#                action_node = PgNode_a(action_nodes)
            for al in action_node.effnodes:
                poss_literals.add(al)
                al.parents.add(action_node)
                action_node.children.add(al)
#                child_nodes.add(al)
#            action_node.children.add(child_nodes)
        self.s_levels.append(poss_literals)
...
#        print('inconsistent_effects_mutex')
#        if node_a1.is_mutex(node_a2):
#            return False
        for eff in node_a1.effnodes:
            for eff1 in node_a2.effnodes:
                if eff.symbol == eff1.symbol and not(eff.is_pos == eff1.is_pos):
                    return True
        return False
...
#        node_a1.show()
        for eff in node_a1.effnodes:
#            is_mutex = False
            for pre in node_a2.prenodes:
                if eff.symbol == pre.symbol and not(eff.is_pos == pre.is_pos):
                    return True
#        if is_mutex:
#            return True
        for eff in node_a2.effnodes:
#            is_mutex = False
            for pre in node_a1.prenodes:
                if eff.symbol == pre.symbol and not(eff.is_pos == pre.is_pos):
                    return True
#            if not is_mutex:
#                return False
...
#        print('competing_needs_mutex')
#        if node_a1.is_mutex(node_a2):
#            return False
#        print('--------------Parent----------')
        for p_a1 in node_a1.parents:
            for p_a2 in node_a2.parents:
                if p_a1.is_mutex(p_a2) or p_a2.is_mutex(p_a1):
                    return True
#            if node_a1.parents.is_mutex(node_a1.parents):
#            return True
#        for pre in node_a1.prenodes:
#            print('pre:',pre.symbol)
#            for pre1 in node_a2.prenodes:
#                print(pre1.symbol,'-->',pre.symbol)
#                print(pre.is_pos,'-->',pre1.is_pos)
#                if pre.symbol == pre1.symbol and not(pre.is_pos == pre1.is_pos):
#                    print('--------------------True----------------------')
#                    return True
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        result = not(node_s1.is_pos == node_s2.is_pos) and node_s1.symbol == node_s2.symbol
        return result
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for ps1 in node_s1.parents:
            isMutex = False
            for ps2 in node_s2.parents:
                if ps1.__eq__(ps2):
                    return False
                if ps1.is_mutex(ps2) or ps2.is_mutex(ps1):
                    isMutex = True
            if not isMutex:
                return False
        return True
...
#            print(str(goal))
            found = False
            for idx,lSet in enumerate(self.s_levels):
                if found:
                    break
                for li in lSet:
                    if li.is_pos:
